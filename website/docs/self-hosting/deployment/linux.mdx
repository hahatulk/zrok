...
- The `zrok2` packages installed. Follow [the Linux installation guide](@zrokdocs/how-tos/install/linux), or on
  Debian/Ubuntu:
...
## Automated bootstrap (Debian/Ubuntu)

:::note
The bootstrap script uses `apt` to install PostgreSQL, RabbitMQ, and InfluxDB and manages systemd service units from
the `.deb` packages. It is designed for Debian-based distributions (Debian, Ubuntu). For RHEL, Fedora, or other
distributions, follow the [manual setup](#manual-setup) below—the manual steps are distro-neutral.
:::
...
The manual steps below work on any Linux distribution with systemd. Only the package installation commands in Step 1
are distro-specific—all subsequent steps use the `zrok2` and `ziti` CLIs, configuration files, and systemd units.
...
1. Install RabbitMQ and PostgreSQL:
    ...
    ```bash
    sudo dnf install rabbitmq-server postgresql-server
    sudo postgresql-setup --initdb   # RHEL/Fedora only
    ```
